...
def preprocess_image(image):
    ...
    retval, thresh = cv2.threshold(blur, thresh_level, 255, cv2.THRESH_OTSU)
    ...
    for i in range(len(counts)):
        if(counts[i]<2500 or counts[i]>5000):
            continue
        ...
        yn=np.max(ind[0])
        height=len(thresh)
        if(yn-ym<xn-xm-12 or ym<height/3):
            continue
        thresh1=thresh.copy()
        thresh1[lab_array!=unique[i]]=0
        # plt.imshow(thresh1)
        # plt.show()
        if(card_count==0):
...
def find_cards(thresh_image):
    ...
    for i in range(len(cnts_sort)):
        ...
        approx = cv2.approxPolyDP(cnts_sort[i], 0.03 * peri, True)
...
def preprocess_card(contour, image):
    ...
    # plt.imshow(qCard.warp)
    # plt.show()
    # Grab corner of warped card image and do a 4x zoom
    Qcorner = qCard.warp[0:CORNER_HEIGHT, 0:CORNER_WIDTH]
    # plt.imshow(Qcorner)
    # plt.show()
    Qcorner_zoom = cv2.resize(Qcorner, (0, 0), fx=2, fy=2)

    # Sample known white pixel intensity to determine good threshold level
    white_level = Qcorner_zoom[15, int((CORNER_WIDTH * 2) / 2)]
    thresh_level = white_level - CARD_THRESH
    if (thresh_level <= 0):
        thresh_level = 1
    retval, query_thresh = cv2.threshold(Qcorner_zoom, thresh_level, 255, cv2.THRESH_OTSU)
    query_thresh=255-query_thresh
    # plt.imshow(query_thresh)
    # plt.show()
    # Split in to top and bottom half (top shows rank, bottom shows suit)
    Qrank = query_thresh[10:205, 10:150]
    Qsuit = query_thresh[205:386, 10:150]
    # cv2.imwrite("10.jpg",Qrank)
    # plt.imshow(Qrank)
    # plt.show()
    # plt.imshow(Qsuit)
    # plt.show()
